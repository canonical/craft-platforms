...
`canonical/craft-platforms` is a Python library that handles platform information for
Snapcraft, Charmcraft, Rockcraft, and other craft apps. A platform is the combination of
an operating system version and its target architecture. The library parses platform
information in a craft project file (e.g. `snapcraft.yaml`) and creates a build plan
manifest describing the environments where the artifact can build and run.

## Craft apps and libraries

craft-platforms is used by craft apps, including but not limited to Charmcraft,
...
Craft apps use craft-platforms in conjunction with the following craft libraries:
...
craft-platforms uses [uv](https://docs.astral.sh/uv/) for dependency management.
