...
def _validate_base_definition(
    ...
    if platform_name:
        ...
        if platform:
            build_entries = [*platform["build-on"], *platform["build-for"]]
            build_on_for_bases = set()
            for entry in [*platform["build-on"], *platform["build-for"]]:
                build_on_for_base = _platforms.get_base_and_architecture(architecture=entry)[0]
                build_on_for_bases.add(str(build_on_for_base) if build_on_for_base else None)
...
def _get_base_from_build_data(
    ...
    if build_base or base:
        return _distro.DistroBase.from_str(build_base or base)

    platform_base, _ = _platforms.get_base_and_name(platform_name=platform_name)
    if platform_base:
        return platform_base

    # build-on and build-for entries should all have the same base, so we only
    # need to check one of them
    if platform:
        build_for_base, _ = _platforms.get_base_and_architecture(
    ...
    raise ValueError(
        "Could not determine the base for the build."
    )
...
def get_platforms_charm_build_plan(
    ...
    for platform_name, platform in platforms.items():
        ...
        if platform is None:
            ...
            try:
                architecture = _architectures.DebianArchitecture(platform_name_without_base)
            except ValueError:
                raise ValueError(
